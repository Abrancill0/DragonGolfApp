...
export default class ScoreHorizontalComponent extends Component {
    ...
    render() {
        ...
        return (
            <View style={{ flex: 1 }}>
                ...
                {this.renderPressTable(f9H, pressesArray)}
            </View>
        );
    }
    ...
    renderPressTable = (holes, pressesArray) => {

        const {
            advTotalStrokes,
        } = this.props;
        console.log('======== pressesArray Horizontal ==========', pressesArray);

        let totalStrokesMA = 0;
        let totalStrokesMB = 0;
        if (advTotalStrokes.length > 0) {
        ...
        const havePress = pressesArray.front9 ? (pressesArray.front9[0].filter(press => press === null).length + pressesArray.back9[0].filter(press => press === null).length) !== 18 : false;
        ...
        if (havePress)
            return (
                <View style={styles.scoreView}>
                    ...
                    <View style={styles.holesView}>
                        {holes.map((hole, i) => <View style={[styles.advView, {
                            borderRightWidth: 0,
                        }]}>
                            {pressesArray.front9.map((press, j) => {
                                if (press[i] !== null) {
                                    const indexPress = press.indexOf(0);
                                    if (indexPress === i && j > 0) return null;
                                    return (<Text style={{ fontSize: 12, color: press[i] < 0 ? Colors.Primary : Colors.Black }}>{press[i] === 0 ? '=' : press[i]}</Text>);
                                } else
                                    return null;
                            })}
                        </View>)}
                        {holes.map((hole, i) => <View style={[styles.advView, {
                            ...
                            {pressesArray.back9.map((press, j) => {
                                if (press[i] !== null) {
                                    const indexPress = press.indexOf(0);
                                    if (indexPress === i && j > 0) return null;
                                    return (<Text key={'press' + j} style={{ fontSize: 12, color: press[i] < 0 ? Colors.Primary : Colors.Black }}>{press[i] === 0 ? '=' : press[i]}</Text>);
                                } else
                                    return null;
                            })}
                        </View>)}
