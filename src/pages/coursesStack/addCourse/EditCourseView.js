...
import { ActualizaCampo } from '../../../Services/Services'
...
class AddCourseView extends Component {
    constructor(props) {
        ...
        if (course){
            ...
            this.state = {
                IDCourse: this.props.route.params.IDCourse,
                name: this.props.route.params.Nombre,
                nameError: '',
                shortName: this.props.route.params.NombreCorto,
                shortNameError: '',
                city: this.props.route.params.Ciudad,
                cityError: '',
                address: '',
                addressError: '',
                country: this.props.route.params.Pais,
                language: 'es'
            }
        //}
    }
    ...
    const token = await AsyncStorage.getItem('usu_id')
        ...
        console.warn(name)
        console.warn(shortName)
        console.warn(city)
        console.warn(country)
        console.warn(token)

        ActualizaCampo(IDCourse, name, shortName, city, country, token)
        .then((res) => {
          console.warn(res)
            if(res.estatus == 1){
                showMessage({
